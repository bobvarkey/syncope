...
export const exportToPDF = (formData: FormData, language: 'en' | 'ml' = 'en') => {
  ...
  const sections = [
    ...
    { title: language === 'ml' ? 'ECG ABCDE സ്ക്രീൻ' : 'ECG ABCDE Screen', data: (formData as any).ecgAbcde?.selectedPatterns },
    { title: language === 'ml' ? 'സിൻകോപ്പ് മരുന്നുകൾ' : 'Medications Implicated in Syncope', data: (formData as any).syncopeMedications },
